...
class SSLVServer(Netscaler):
    ...
    def get_sslvserver_vservername(self, proposed_config):

        attrs_list = proposed_config.keys()
        attrs = "?attrs=" + ",".join(attrs_list)
        url = self.url + self.api_endpoint + "/" + proposed_config["vservername"] + attrs
        response = self.session.get(url, headers=self.headers, verify=self.verify)

        return response.json().get("sslvserver", [])

    def unset_sslvserver_tls_protocol(self, module, new_config):

        config = []

        config_state="unset"
        url = self.url + self.api_endpoint + "?action={}".format(config_state)
        config.append({"method": "post", "url": url, "body": { self.api_endpoint:{ "vservername:{},ssl3:{},tls1:{}".format(new_config["vservername"], "true" , "true")}}})

        #url = self.url + "sslvserver?action=unset"
        #config.append({"method": "post", "url": url, "body": { self.api_endpoint:{new_config}}})
        #config.append({"method": "post", "url": url, "body": { self.api_endpoint:{ "vservername":new_config["vservername"],"ssl3":new_config["ssl3"],"tls1":new_config["tls1"]}}})

        #config.append({"method": "post", "url": url, "body": new_config }})

        return config

        #config = []
        #url = self.url + self.api_endpoint + "?action=unset"
        #config.append({"method": "post", "url": url, "body": { self.api_endpoint:{ "vservername":new_config["vservername"],"ssl3":"true","tls1":"true"}}})
        #url = self.url + self.api_endpoint + "?action={}".format(config_state)
        #return config
...
def change_config(session, module, proposed, all_existing):
    ...
    #if all_existing:
        changed = True
        config = session.config_update(module, proposed)
    else:
        existing = proposed

    #return dict(all_existing=all_existing, changed=changed, config=config)
    return dict(changed=changed, config=config, existing=existing, all_existing=all_existing)

def unset_sslvserver(session, module, proposed, all_existing):
    ...
    #if all_existing:
        changed = True
        config = session.unset_sslvserver_tls_protocol(module, proposed)
    else:
        existing = proposed
