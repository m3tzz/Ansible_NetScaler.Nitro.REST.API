...
class Netscaler(object):
    ...
    def change_name(self, existing_name, proposed_name):
        ...
        body = {self.api_endpoint: {"vservername": existing_name, "newname":proposed_name}}
    ...
    def change_state(self, object_name, state):
        ...
        body = {self.api_endpoint: {"vservername": object_name}}
        response = self.session.post(url, json=body, headers=self.headers, verify=self.verify)
    ...
    def config_rename(self, module, existing_name, proposed_name):
        ...
        rename_config = {"vservername": existing_name, "newname": proposed_name}
    ...
    def config_update(self, module, update_config):
        ...
        the "vservername" key (len > 1). If there are more updates to make, the put_config method is used to push those to the
        Netscaler. Note that this method uses the common "vservername" key to input to the change_state method; if the API
        ...
        if "state" in update_config:
            ...
            if not module.check_mode:
                config_status = self.change_state(update_config["vservername"], config_state)
                if config_status.ok:
                    config.append({"method": "post", "url": config_status.url, "body": {"vservername": update_config["vservername"]}})
                else:
                    logout = self.logout()
                ...
                config.append({"method": "post", "url": url, "body": {"vservername": update_config["vservername"]}})
    ...
    def get_diff(proposed, existing):
        ...
        if diff == proposed:
            ...
            diff["vservername"] = proposed["vservername"]
            return "update", diff
        else:
            return "none", {}
    ...
    def get_sslvserver_stats(self):
        """
        This method is used to get the sslvserver statistical info for all vservers on the Netscaler.
        :return: A list of dictionaries for all statistical data for the sslvservers. An empty dictionary is returned if
                 the request is unsuccessful.
        """
        response = self.session.get(self.stat_url + "sslvserver", headers=self.headers, verify=self.verify)

        return response.json().get("sslvserver", [])
...
class SSLVServer(Netscaler):
    """
    This is the class used for interacting with the "sslvserver" API endpoint. In addition to sslvserver specific methods,
    the api endpoint default value is set to "sslvserver."
    """

    def __init__(self, host, user, passw, secure=True, verify=False, api_endpoint="sslvserver", **kwargs):
        super(SSLVServer, self).__init__(host, user, passw, secure, verify, api_endpoint, **kwargs)

    def add_sslvserver_sslciphersuite_binding(self, module, new_config):

        config = []

        if not module.check_mode:
            config_status = self.bind_sslciphersuite(new_config)
            if config_status.ok:
                config.append({"method": "post", "url": config_status.url, "body": new_config})
            else:
                logout = self.logout()
                module.fail_json(msg="Unable to Bind CihperSuite", netscaler_response=config_status.json(), logout=logout.ok)
        else:
            config.append({"method": "post", "url": self.url + "_sslciphersuite_binding", "body": new_config})

        return config

    def bind_sslciphersuite(self, new_config):

        url = self.url + self.api_endpoint + "_sslciphersuite_binding"
        body = {self.api_endpoint + "_sslciphersuite_binding": new_config}
        response = self.session.post(url, json=body, headers=self.headers, verify=self.verify)
    ...
    def get_sslvserver_sslciphersuite_binding(self, proposed_config):

        attrs_list = proposed_config.keys()
        attrs = "?attrs=" + ",".join(attrs_list)
        url = self.url + "sslvserver_sslciphersuite_binding/" + proposed_config["vservername"] + attrs
        response = self.session.get(url, headers=self.headers, verify=self.verify)

        return response.json().get("sslvserver_sslciphersuite_binding", [])

    def remove_sslvserver_sslciphersuite_binding(self, module, new_config):

        config = []

        if not module.check_mode:
            config_status = self.unbind_sslciphersuite(new_config)
            if config_status.ok:
                config.append({"method": "delete", "url": config_status.url, "body": {}})
            else:
                logout = self.logout()
                module.fail_json(msg="Unable to Remove sslciphersuite", netscaler_response=config_status.json(), logout=logout.ok)
        else:
            ...
            config.append({"method": "delete", "url": self.url + "sslvserver_sslciphersuite_binding" + args, "body": {}})

        return config

    def unbind_sslciphersuite(self, binding_config):
        ...
        url = self.url + "sslvserver_sslciphersuite_binding" + args
        response = self.session.delete(url, headers=self.headers, verify=self.verify)
...
def main():
    argument_spec = dict(
        ...
        ciphername=dict(required=False, type="str")
    ...
    ciphername = module.params["ciphername"]
    if ciphername:
        ciphername = ciphername.upper()
    ...
    argument_check = dict(host=host, vservername=args["vservername"],ciphername=args["ciphername"])
    ...
    session = SSLVServer(host, username, password, use_ssl, validate_certs, **kwargs)
    ...
    #all_existing = session.get_sslvserver_sslciphersuite_binding(proposed["vservername"])

    all_existing = session.get_sslvserver_sslciphersuite_binding(proposed)

    if state == "present":
        results = change_config(session, module, proposed, all_existing)
    else:
        results = delete_sslvserver_sslciphersuite_binding(session, module, proposed, all_existing)
...
def change_config(session, module, proposed, all_existing):
    ...
    if proposed not in all_existing:
        changed = True
        config = session.add_sslvserver_sslciphersuite_binding(module, proposed)
        existing = {}
    else:
        existing = proposed

    return dict(all_existing=all_existing, changed=changed, config=config, existing=existing)
    #return dict(changed=changed, config=config, existing=existing,all_existing=all_existing)

def delete_sslvserver_sslciphersuite_binding(session, module, proposed, all_existing):
    ...
    if proposed in all_existing:
        changed = True
        config = session.remove_sslvserver_sslciphersuite_binding(module, proposed)
        existing = proposed
    else:
        existing = {}
