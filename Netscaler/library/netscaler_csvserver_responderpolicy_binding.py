...
class CSVServer(Netscaler):
    ...
    def add_csvserver_responderpolicy_binding(self, module, new_config):
        config = []
        ...
        if not module.check_mode:
            ...
            config.append({"method": "post", "url": self.url + self.api_endpoint + "_responderpolicy_binding",
                           "body": new_config})
    ...
    def bind_responderpolicy(self, new_config):
        ...
        url = self.url + self.api_endpoint + "_responderpolicy_binding"
        body = {self.api_endpoint + "_responderpolicy_binding": new_config}
        response = self.session.post(url, json=body, headers=self.headers, verify=self.verify)
    ...
    def get_csvserver_responderpolicy_binding(self, object_name):

        attrs_list = object_name.keys()
        attrs = "?attrs=" + ",".join(attrs_list)
        url = self.url + "csvserver_responderpolicy_binding/" + object_name["name"] + attrs
        response = self.session.get(url, headers=self.headers, verify=self.verify)

        return response.json().get("csvserver_responderpolicy_binding", [])

        #url = self.url + self.api_endpoint + "_csvserver_responderpolicy_binding/" + object_name + "?filter=name,policyname"
        #response = self.session.get(url, headers=self.headers, verify=self.verify)
        #return response.json().get("csvserver_responderpolicy_binding", [])

    def remove_csvserver_responderpolicy_binding(self, module, new_config):
        ...
        if not module.check_mode:
            ...
            if config_status.ok:
                ...
                logout = self.logout()
                module.fail_json(msg="Unable to Remove responderpolicy", netscaler_response=config_status.json(), logout=logout.ok)
        else:
            ...
            config.append({"method": "delete", "url": self.url + "csvserver_responderpolicy_binding" + args, "body": {}})
        ...
        #else:
        #    url = self.url + self.api_endpoint + "_csvserver_responderpolicy_binding?args=name:{},policyname:{}".format(
    ...
    def unbind_responderpolicy(self, binding_config):
        ...
        url = self.url + "csvserver_responderpolicy_binding" + args
        response = self.session.delete(url, headers=self.headers, verify=self.verify)
...
def main():
    ...
    if state == "present":
        ...
        results = delete_csvserver_responderpolicy_binding(session, module, proposed, all_existing)
...
def change_config(session, module, proposed, all_existing):
    ...
    if proposed not in all_existing:
        changed = True
        config = session.add_csvserver_responderpolicy_binding(module, proposed)
        existing = {}
    else:
        existing = proposed
...
def delete_csvserver_responderpolicy_binding(session, module, proposed, all_existing):
    ...
    if proposed in all_existing:
        changed = True
        config = session.remove_csvserver_responderpolicy_binding(module, proposed)
        existing = proposed
    else:
        existing = {}
