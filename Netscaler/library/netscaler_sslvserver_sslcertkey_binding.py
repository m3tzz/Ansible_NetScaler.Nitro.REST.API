...
class SSLVServer(Netscaler):
    ...
    def add_sslvserver_sslcertkey_binding(self, module, new_config):
        ...
        if not module.check_mode:
            ...
            config.append({"method": "post", "url": self.url + self.api_endpoint + "_sslcertkey_binding", "body": new_config})

        return config

    def bind_certkey(self, new_config):

        url = self.url + self.api_endpoint + "_sslcertkey_binding"
        body = {self.api_endpoint + "_sslcertkey_binding": new_config}
        response = self.session.post(url, json=body, headers=self.headers, verify=self.verify)
    ...
    def get_sslvserver_sslcertkey_binding(self, object_name):
        ...
        """
        attrs_list = object_name.keys()
        attrs = "?attrs=" + ",".join(attrs_list)
        url = self.url + "sslvserver_sslcertkey_binding/" + object_name["vservername"] + attrs
        response = self.session.get(url, headers=self.headers, verify=self.verify)

        return response.json().get("sslvserver_sslcertkey_binding", [])

        #url = self.url + self.api_endpoint + "_sslcertkey_binding/" + object_name + "?filter=vservername,certkeyname"
        #response = self.session.get(url, headers=self.headers, verify=self.verify)
        #return response.json().get("sslvserver_sslcertkey_binding", [])

    def remove_sslvserver_sslcertkey_binding(self, module, new_config):
        ...
        #config = []
        #if not module.check_mode:
        #    config_status = self.unbind_responderpolicy(new_config)
        #    if config_status.ok:
        #        config.append({"method": "delete", "url": config_status.url, "body": {}})
        #    else:
        #        logout = self.logout()
        #        module.fail_json(msg="Unable to Remove Responder Policy Binding", netscaler_response=config_status.json(), logout=logout.ok)
        #else:
        #    url = self.url + self.api_endpoint + "_sslcertkey_binding?args=vservername:{},certkeyname:{}".format(
        #        new_config["vservername"], new_config["certkeyname"])
        #    config.append({"method": "delete", "url": url, "body": {}})
    ...
    def unbind_certkey(self, binding_config):
        ...
        #url = self.url + self.api_endpoint + "_sslcertkey_binding?args=vservername:{},certkeyname:{}".format(
        #    binding_config["name"], binding_config["certkeyname"]
        #)
        #response = self.session.delete(url, headers=self.headers, verify=self.verify)
        #return response
...
def main():
    ...
    # check for required values, this allows all values to be passed in provider
    argument_check = dict(host=host, certkeyname=args["certkeyname"], vserver_name=args["vservername"])
    for key, val in argument_check.items():
        if not val:
            module.fail_json(msg="The {} parameter is required".format(key))
    ...
    if state == "present":
        ...
        results = delete_sslvserver_sslcertkey_binding(session, module, proposed, all_existing)
...
def change_config(session, module, proposed, all_existing):
    """
    The purpose of this function is to determine if the binding needs to be pushed to the Netscaler. A new sslvserver
    to sslcertkey binding will be configured if the binding does not currently exist.
    ...
    if proposed not in all_existing:
        changed = True
        config = session.add_sslvserver_sslcertkey_binding(module, proposed)
        existing = {}
    else:
        existing = proposed
...
def delete_sslvserver_sslcertkey_binding(session, module, proposed, all_existing):

    """
    The purpose of this function is to delete a sslvserver to sslcertkey binding from the Netscaler. Checks are
    ...
    if proposed in all_existing:
        changed = True
        config = session.remove_sslvserver_sslcertkey_binding(module, proposed)
        existing = proposed
    else:
        existing = {}
