...
short_description: Manages CS VServer resources and attributes.
description:
  - Manages Netscaler CS VServer configurations using Nitro API.
options:
  host:
    description:
  ...
  backup_csvserver:
    description:
      - The name of the backup csvserver
    required: false
    type: str
  client_timeout:
    description:
  ...
  comment:
    description:
      - A comment about the csvserver
    required: false
    type: str
  config_override:
    description:
      - Setting to True enables changing IP Addresses and Names
      - If an CS VServer with a different name is configured with the same IP Address,
        Traffic Domain, and Port, then the existing VServer will be renamed.
      - If an CS VServer already exists with the same Name, but different IP Address,
        then the existing VServer will have its IP Address updated.
    required: false
    type: bool
  conn_failover:
    description:
      - The csvserver connection setting
    required: false
    type: str
  ...
  csvserver_name:
    description:
      - The name of the csvserver object
    required: true
    type: str
  csvserver_port:
    description:
      - The port the csvserver will listen on.
  ...
  persistence:
    description:
      - The persistence type used by the csvserver.
    required: false
    type: str
  ...
  service_type:
    description:
      - The type of service the csvserver provides.
    required: false
    type: str
...
EXAMPLES = '''
- name: Config csvserver Object
  netscaler_csvserver:
    host: "{{ inventory_hostname }}"
    username: "{{ username }}"
    password: "{{ password }}"
    csvserver_name: "lbvsvr_app01"
    ip_address: "10.10.10.21"
    service_type: "any"
    csvserver_port: "*"
- name: Config Backup csvserver Object
  netscaler_csvserver:
    host: : "{{ inventory_hostname }}"
    username: "{{ username }}"
    password: "{{ password }}"
    csvserver_name: "lbvsvr_app02"
    ip_address: "0.0.0.0"
    service_type: "SSL"
    csvserver_port: "0"
    port: 8080
...
- name: Config Server Object in Lab Partition
  netscaler_csvserver:
    ...
    password: "{{ password }}"
    csvserver_name: "lbvsvr_app02"
    backup_csvserver: "lbvsvr_app02_backup"
    ip_address: "10.10.10.22"
    csvserver_port: "443"
    use_ssl: False
    port: 8080
    comment: app02 vserver
- name: Delete csvserver Object
  netscaler_csvserver:
    host: "{{ inventory_hostname }}"
    username: "{{ username }}"
    password: "{{ password }}"
...
RETURN = '''
existing:
    description: The existing configuration for the csvserver (uses csvserver_name) before the task executed.
    returned: always
    type: dict
...
config:
    ...
    returned: always
    type: list
    sample: [{"method": "post", "url": "https://netscaler/nitro/v1/config/csvserver?action=disable",
...
class Netscaler(object):
    ...
    def get_csvserver_stats(self):
        """
        This method is used to get the csvserver statistical info for all vservers on the Netscaler.
        :return: A list of dictionaries for all statistical data for the csvservers. An empty dictionary is returned if
                 the request is unsuccessful.
        """
        response = self.session.get(self.stat_url + "csvserver", headers=self.headers, verify=self.verify)
...
class CSVServer(Netscaler):
    """
    This is the class used for interacting with the "csvserver" API endpoint. In addition to csvserver specific methods,
    the api endpoint default value is set to "csvserver."
    """

    def __init__(self, host, user, passw, secure=True, verify=False, api_endpoint="csvserver", **kwargs):
        super(CSVServer, self).__init__(host, user, passw, secure, verify, api_endpoint, **kwargs)
    ...
    def add_csvserver_lbvserver_binding(self, module, new_config):
        """
        This method is used to handle the logic for Ansible modules when the "state" is set to "present" and the
        proposed binding is new. The bind_csvserver_csvserver_binding method is used to post the binding configuration to the
        Netscaler.
        :param module: The AnsibleModule instance started by the task.
        :param new_config: Type dict.
                           The binding configuration to send to the Nitro API.
        ...
        if not module.check_mode:
            config_status = self.bind_lbvserver(new_config)
            if config_status.ok:
                config.append({"method": "post", "url": config_status.url, "body": new_config})
            else:
                logout = self.logout()
                module.fail_json(msg="Unable to Bind lbvserver", netscaler_response=config_status.json(), logout=logout.ok)
        else:
            config.append({"method": "post", "url": self.url + self.api_endpoint + "_csvserver_lbvserver_binding",
                           "body": new_config})
    ...
    def bind_lbvserver(self, new_config):
        """
        This method is used to submit a binding request to the Netscaler using the Nitro API. It is expected that you
        ...
        :return: The response from the request to add the binding.
        """
        url = self.url + self.api_endpoint + "_csvserver_lbvserver_binding"
        body = {self.api_endpoint + "_csvserver_lbvserver_binding": new_config}
        response = self.session.post(url, json=body, headers=self.headers, verify=self.verify)
    ...
    def get_certkey_bindings(self, proposed_config):
        """
        This method is used to get the csvserver's current certkey bindings.
        :param proposed_config: Type dict.
                                The proposed lbvserver to sslcertkey binding.
        :return: A list of dictionaries of the current certificates bound to the csvserver object. If no bindings
    ...
    def get_csvserver_by_ip_td_port(self, ip_address, traffic_domain, port):
        """
        ...
        return response.json().get("csvserver", [{}])[0]

    def get_csvserver_lbvserver_binding(self, object_name):
        """
        ...
        :return: A list of dictionaries of the current servicegroups bound to the lbvserver object. If no bindings
                 exist, then an empty dictionary is returned.
        """
        url = self.url + self.api_endpoint + "_csvserver_lbvserver_binding/" + object_name + "?attrs=name,lbvserver"
        response = self.session.get(url, headers=self.headers, verify=self.verify)

        return response.json().get("csvserver_lbvserver_binding", [])
    ...
    def remove_csvserver_lbvserver_binding(self, module, new_config):
        """
        This method is used to handle the logic for Ansible modules when the "state" is set to "absent" and the
        ...
        if not module.check_mode:
            config_status = self.unbind_lbvserver(new_config)
            if config_status.ok:
                config.append({"method": "delete", "url": config_status.url, "body": {}})
            else:
                logout = self.logout()
                module.fail_json(msg="Unable to Remove LBVserver Binding", netscaler_response=config_status.json(), logout=logout.ok)
        else:
            url = self.url + self.api_endpoint + "_csvserver_lbvserver_binding?args=name:{},lbvserver:{}".format(
                new_config["name"], new_config["lbvserver"])
    ...
    def unbind_lbvserver(self, binding_config):
        """
        This method is used to remove a servicegroup binding from a lbvserver object. It is expected that you check
        that the binding currently exists with the get_csvserver_lbvserver_binding method.
        :param binding_config: Type dict.
                               The current binding config that needs to be removed.
        :return: The response from the request to unbind thelbvserver from the csvserver.
        """
        url = self.url + self.api_endpoint + "_csvserver_lbvserver_binding?args=name:{},lbvserver:{}".format(
            binding_config["name"], binding_config["lbvserver"]
        )
        response = self.session.delete(url, headers=self.headers, verify=self.verify)
...
def main():
    argument_spec = dict(
        ...
        backup_csvserver=dict(required=False, type="str"),
        comment=dict(required=False, type="str"),
        ip_address=dict(required=False, type="str"),
        csvserver_name=dict(required=False, type="str"),
        csvserver_port=dict(required=False, type="str"),
        csvserver_state=dict(choices=["disabled", "enabled"], required=False, type="str"),
        #sopersistence=dict(choices=VALID_PERSISTENCE_TYPES, required=False, type="str"),
    ...
    csvserver_port = module.params["csvserver_port"]
    if csvserver_port:
        csvserver_port = str(csvserver_port)
    csvserver_state = module.params["csvserver_state"]
    if csvserver_state:
        csvserver_state = csvserver_state.upper()
    ...
    if traffic_domain:
        ...
        traffic_domain = "0"
    lbvserver_name = module.params["lbvserver_name"]
    if lbvserver_name:
        lbvserver_name = lbvserver_name.upper()

    args = dict(
        appflowlog=app_flow_log,
        backupvserver=module.params["backup_csvserver"],
        comment=module.params["comment"],
        ipv46=module.params["ip_address"],
        name=module.params["csvserver_name"],
        port=csvserver_port,
        state=csvserver_state,
        servicetype=service_type,
        td=traffic_domain,
        lbvserver=module.params["lbvserver_name"]
    )
    ...
    if "port" in proposed:
        if proposed["port"] == "*":
            ...
            try:
                ...
                module.fail_json(msg="'csvserver_port' Must be a Number from 0 to 65535, or '*'")

    kwargs = dict()
    if port:
        kwargs["port"] = port

    session = CSVServer(host, username, password, use_ssl, validate_certs, **kwargs)
    ...
    if state == "present":
        ...
        results = delete_csvserver(session, module, proposed["name"], existing)
...
def change_config(session, module, proposed, existing):
    ...
    if "ipv46" in config_diff and config_diff["ipv46"] != "0.0.0.0" and "port" in config_diff:
        dup_csvserver = session.get_csvserver_by_ip_td_port(proposed["ipv46"], proposed["td"], proposed["port"])
        if dup_csvserver and not module.params["config_override"]:
            dup_dict = dict(
                proposed_name=proposed["name"], existing_name=dup_csvserver["name"], ip_address=dup_csvserver["ipv46"],
                traffic_domain=dup_csvserver["td"], service_type=dup_csvserver["servicetype"], port=dup_csvserver["port"],
                partition=module.params["partition"]
            )
            session_logout = session.logout()
            module.fail_json(msg="Changing a CS VServer's Name requires setting the config_override param to True:.", conflict=dup_dict, logout=session_logout.ok)
        elif dup_csvserver:
           changed = True
           rename = session.config_rename(module, dup_csvserver["name"], proposed["name"])
    ...
    if config_method == "new":
        # raise error if new primary csvserver does not include proper port and servicetype configurations
        if "ipv46" in config_diff and config_diff["ipv46"] != "0.0.0.0":
            if "port" not in config_diff or "servicetype" not in config_diff:
                session_logout = session.logout()
                module.fail_json(msg="The Port and Service Type must be specified when creating a new Primary CS VServer.", logout=session_logout.ok)
        ...
        #    module.fail_json(msg="An IP Address value of '0.0.0.0' only supports a Port of 0", logout=session_logout.ok)
            ...
            session_logout = session.logout()
            module.fail_json(msg="The IP Address must be specified when creating a new Primary CS VServer", logout=session_logout.ok)
        ...
        if "servicetype" in config_diff:
            ...
            module.fail_json(msg="Modifying the Service Type is not Supported. This can be achieved by first deleting the "
                                 "CS VServer, and then creating a CS VServer with the changes.", conflict=conflict, logout=session_logout.ok)
        ...
        if "td" in config_diff:
            ...
            session_logout = session.logout()
            module.fail_json(msg="Updating a CS VServer's Traffic Domain is not Supported. This can be achieved by first deleting the "
                                 "CS VServer, and then creating a CS VServer with the changes.", conflict=conflict, logout=session_logout.ok)
        ...
        if "port" in config_diff:
            ...
            session_logout = session.logout()
            module.fail_json(msg="Modifying the CS VServer's Port is not Supported. This can be achieved by first deleting the "
                                 "CS VServer, and then creating a CS VServer with the changes.", conflict=conflict, logout=session_logout.ok)
        ...
        if "ipv46" in config_diff and not module.params["config_override"]:
            dup_dict = dict(name=proposed["name"], proposed_ip=proposed["ipv46"], existing_ip=existing["ipv46"], traffic_domain=proposed["td"],
                            partition=module.params["partition"])
            session_logout = session.logout()
            module.fail_json(msg="Updating a CS VServer's IP Addresses requires setting the config_override param to True.", conflict=dup_dict, logout=session_logout.ok)
...
def delete_csvserver(session, module, proposed_name, existing ):
    """
    The purpose of this function is to delete a csvserver from the Netscaler. Checks are currently only done to ensure
    the object exists before submitting it for deletion. If the csvserver has bindings, it will still be deleted.
    :param session: The Server instance that has an established session with the Netscaler.
    :param module: The AnsibleModule instance
    :param proposed_name: Type str.
