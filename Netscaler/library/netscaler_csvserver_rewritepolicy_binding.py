...
class CSVServer(Netscaler):
    ...
    def add_csvserver_rewritepolicy_binding(self, module, new_config):
        config = []

        if not module.check_mode:
            config_status = self.bind_responderpolicy(new_config)
            if config_status.ok:
                config.append({"method": "post", "url": config_status.url, "body": new_config})
            else:
                logout = self.logout()
                module.fail_json(msg="Unable to bind ResponderPolicy", netscaler_response=config_status.json(), logout=logout.ok)
        else:
            config.append({"method": "post", "url": self.url + self.api_endpoint + "_rewritepolicy_binding",
                           "body": new_config})

        return config

    def bind_responderpolicy(self, new_config):

        url = self.url + self.api_endpoint + "_rewritepolicy_binding"
        body = {self.api_endpoint + "_rewritepolicy_binding": new_config}
        response = self.session.post(url, json=body, headers=self.headers, verify=self.verify)
    ...
    def get_csvserver_rewritepolicy_binding(self, object_name):

        attrs_list = object_name.keys()
        attrs = "?attrs=" + ",".join(attrs_list)
        url = self.url + "csvserver_rewritepolicy_binding/" + object_name["name"] + attrs
        response = self.session.get(url, headers=self.headers, verify=self.verify)

        return response.json().get("csvserver_rewritepolicy_binding", [])

        #url = self.url + self.api_endpoint + "_csvserver_rewritepolicy_binding/" + object_name + "?filter=name,policyname"
        #response = self.session.get(url, headers=self.headers, verify=self.verify)
        #return response.json().get("csvserver_rewritepolicy_binding", [])

    def remove_csvserver_rewritepolicy_binding(self, module, new_config):

        config = []

        if not module.check_mode:
            config_status = self.unbind_rewritepolicy(new_config)
            if config_status.ok:
                config.append({"method": "delete", "url": config_status.url, "body": {}})
            else:
                logout = self.logout()
                module.fail_json(msg="Unable to unbind rewrite policy", netscaler_response=config_status.json(), logout=logout.ok)
        else:
            ...
            config.append({"method": "delete", "url": self.url + "csvserver_rewritepolicy_binding" + args, "body": {}})
        ...
        #else:
        #    url = self.url + self.api_endpoint + "_csvserver_rewritepolicy_binding?args=name:{},policyname:{}".format(
        #        new_config["name"], new_config["policyname"])

        #    config.append({"method": "delete", "url": url, "body": {}})

        #return config

    def unbind_rewritepolicy(self, binding_config):

        #url = self.url + self.api_endpoint + "_csvserver_rewritepolicy_binding?args=name:{},policyname:{}".format(
        #    binding_config["name"], binding_config["policyname"]
        #)
        #response = self.session.delete(url, headers=self.headers, verify=self.verify)
        #return response
        ...
        url = self.url + "csvserver_rewritepolicy_binding" + args
        response = self.session.delete(url, headers=self.headers, verify=self.verify)
...
def main():
    argument_spec = dict(
        ...
        rp_gotopriorityexpression=dict(required=False, type="str"),
        rp_bindpoint=dict(choices=['REQUEST', 'RESPONSE'], required=False, type="str"),
    ...
    all_existing = session.get_csvserver_rewritepolicy_binding(proposed)

    if state == "present":
        results = change_config(session, module, proposed, all_existing)
    else:
        results = delete_csvserver_rewritepolicy_binding(session, module, proposed, all_existing)
...
def change_config(session, module, proposed, all_existing):
    ...
    if proposed not in all_existing:
        changed = True
        config = session.add_csvserver_rewritepolicy_binding(module, proposed)
        existing = {}
    else:
        existing = proposed
...
def delete_csvserver_rewritepolicy_binding(session, module, proposed, all_existing):
    ...
    if proposed in all_existing:
        changed = True
        config = session.remove_csvserver_rewritepolicy_binding(module, proposed)
        existing = proposed
    else:
        existing = {}
