DOCUMENTATION = '''
---
module: netscaler_lbvserver_certkey
...
options:
  ...
  cert_key_name:
    description:
      - The name of the cert key to bind to the lbvserver.
    required: true
    type: str
  crl_check:
    description:
...
RETURN = '''
all_existing:
    description: All existing cert key bindings for the lbvserver.
    returned: always
    type: list
    sample: []
config:
    ...
    returned: always
    type: dict
    sample: {"vservername": "vserver_app01", "certkey": "svcgrp_app01"}
existing:
    description: The existing binding for the lbvserver and cert key pair. New bindings return an empty dict.
    returned: always
    type: dict
    sample: {}
logout:
...
def main():
    ...
    if state == "present":
        ...
        results = delete_certkey_binding(session, module, proposed, all_existing)
...
def change_config(session, module, proposed, all_existing):
    ...
    if proposed not in all_existing:
        changed = True
        config = session.add_certkey_binding(module, proposed)
        existing = {}
    else:
        existing = proposed
...
def delete_certkey_binding(session, module, proposed, all_existing):
    """
    ...
    if proposed in all_existing:
        changed = True
        config = session.remove_certkey_binding(module, proposed)
        existing = proposed
    else:
        existing = {}
